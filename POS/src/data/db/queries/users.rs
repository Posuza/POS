...
pub async fn create_user(
    ...
    let password_hash =
        hash(password, 12).map_err(|e| sqlx::Error::Configuration(e.to_string().into()))?;
...
pub async fn get_user_by_username(
    ...
    sqlx::query_as::<_, UserDB>("SELECT * FROM users WHERE username = ?")
        .bind(username)
        .fetch_optional(db)
        .await
}
...
pub async fn verify_password(
    ...
    match get_user_by_username(db, username).await {
        Ok(Some(user)) => verify(password, &user.password_hash).map_err(|e| e.to_string()),
        Ok(None) => Ok(false),
...
pub async fn get_all_users(db: &SqlitePool) -> Result<Vec<UserDB>, sqlx::Error> {
    sqlx::query_as::<_, UserDB>("SELECT * FROM users ORDER BY created_at DESC")
        .fetch_all(db)
        .await
}
