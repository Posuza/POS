...
pub fn validate_username(username: &str) -> Result<()> {
    if username.is_empty() {
        return Err(PosError::ValidationError("Username cannot be empty".to_string()));
    }
    
    if username.len() < USERNAME_MIN_LENGTH {
        return Err(PosError::ValidationError(
            format!("Username must be at least {} characters", USERNAME_MIN_LENGTH)
        ));
    }
    
    if username.len() > USERNAME_MAX_LENGTH {
        return Err(PosError::ValidationError(
            format!("Username must not exceed {} characters", USERNAME_MAX_LENGTH)
        ));
    }
...
pub fn validate_email(email: &str) -> Result<()> {
    if email.is_empty() {
        return Err(PosError::ValidationError("Email cannot be empty".to_string()));
    }
    
    if email.len() > EMAIL_MAX_LENGTH {
        return Err(PosError::ValidationError(
            format!("Email must not exceed {} characters", EMAIL_MAX_LENGTH)
        ));
    }
    
    if !email.contains('@') || !email.contains('.') {
        return Err(PosError::ValidationError("Invalid email format".to_string()));
    }
...
pub fn validate_password(password: &str) -> Result<()> {
    if password.is_empty() {
        return Err(PosError::ValidationError("Password cannot be empty".to_string()));
    }
    
    if password.len() < PASSWORD_MIN_LENGTH {
        return Err(PosError::ValidationError(
            format!("Password must be at least {} characters", PASSWORD_MIN_LENGTH)
        ));
    }
    
    if password.len() > PASSWORD_MAX_LENGTH {
        return Err(PosError::ValidationError(
            format!("Password must not exceed {} characters", PASSWORD_MAX_LENGTH)
        ));
    }
...
pub fn validate_barcode(barcode: &str) -> Result<()> {
    if barcode.is_empty() {
        return Err(PosError::ValidationError("Barcode cannot be empty".to_string()));
    }
    
    if barcode.len() < BARCODE_MIN_LENGTH {
        return Err(PosError::ValidationError(
            format!("Barcode must be at least {} characters", BARCODE_MIN_LENGTH)
        ));
    }
    
    if barcode.len() > BARCODE_MAX_LENGTH {
        return Err(PosError::ValidationError(
            format!("Barcode must not exceed {} characters", BARCODE_MAX_LENGTH)
        ));
    }
...
pub fn validate_price(price: f32) -> Result<()> {
    if price < 0.0 {
        return Err(PosError::ValidationError("Price cannot be negative".to_string()));
    }
...
pub fn validate_quantity(quantity: i32) -> Result<()> {
    if quantity < 0 {
        return Err(PosError::ValidationError("Quantity cannot be negative".to_string()));
    }
    
    if quantity > 1_000_000 {
        return Err(PosError::ValidationError("Quantity is too high".to_string()));
    }
...
pub fn validate_image_format(format: &str) -> Result<()> {
    if SUPPORTED_IMAGE_FORMATS.contains(&format) {
        ...
        Err(PosError::ValidationError(
            format!("Unsupported image format: {}. Supported: {:?}", format, SUPPORTED_IMAGE_FORMATS)
        ))
    }
}
