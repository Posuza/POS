...
pub(crate) use crate::utils::formatters::{format_price, format_datetime};
...
pub(crate) fn image_preview_from_filename(name: &str, image_type: &Option<String>, folder: &str) -> Option<String> {
...
pub(crate) fn save_sales_to_json(sales: &[crate::data::json_store::SaleRecord]) -> Result<(), String> {
    let settings = get_settings();
...
pub(crate) fn export_sales_csv(sales: &[crate::data::json_store::SaleRecord]) -> Result<String, String> {
    let settings = get_settings();
    let export_dir = std::path::PathBuf::from(settings.json_data_path).join("exports");
    std::fs::create_dir_all(&export_dir).map_err(|e| format!("Create export dir failed: {}", e))?;
...
pub(crate) fn export_staff_csv(users: &[crate::data::json_store::UserRecord]) -> Result<String, String> {
    let settings = get_settings();
    let export_dir = std::path::PathBuf::from(settings.json_data_path).join("exports");
    std::fs::create_dir_all(&export_dir).map_err(|e| format!("Create export dir failed: {}", e))?;
...
pub(crate) fn normalize_sales_statuses(sales: &mut [crate::data::json_store::SaleRecord]) {
    for sale in sales.iter_mut() {
        ...
        let normalized = if status.contains("paid") || status.contains("completed") {
            ...
            "voided"
        } else if status.contains("pending") || status.contains("due") || status.contains("processing") {
            "pending"
        } else {
            status.as_str()
        };
